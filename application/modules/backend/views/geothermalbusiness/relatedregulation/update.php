...
<div class="content-wrapper">
  <section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1>Change Related Regulation</h1>
        </div>
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item active">Change Related Regulation</li>
          </ol>
        </div>
      </div>
    </div><!-- /.container-fluid -->
      ...
      <div class="row">
        <div class="col-md-12">
            <div class="card card-primary">
              ...
              <form action="<?= site_url('backend/geothermalbusiness/relatedregulation/update/down/'.$datanya->id_related_regulation);?>" method="post" enctype="multipart/form-data">
                ...
                <?php endif; ?>
                <div class="card-body">
                  <?php if(form_error('id_kategori_related') == TRUE): ?>
                    <div class="form-group">
                      <label class="control-label" for="inputWarning"><i class="fa fa-bell"></i> Category<b>&nbsp;<?= strip_tags(form_error('id_kategori_related'));?></b></label>
                      <select name="id_kategori_related" class="form-control select2" required="required">
                        <option value="none" hidden="hidden">-- SELECT CATEGORY --</option>
                        <?php
                          $category = $this->db->get('category_related_regulation');
                          foreach($category->result() as $no => $category):
                        ?>
                        <option value="<?= $category->id_kategori_related;?>"><?= $category->nama_kategori;?></option>
                        <?php endforeach; ?>
                      </select>
                    </div>
                  <?php else: ?>
                    <div class="form-group">
                      ...
                      <select name="id_kategori_related" class="form-control select2" required="required">
                        ...
                        <?php
                          ...
                          foreach($category->result() as $no => $category):
                        ?>
                        <option <?= ($datanya->id_kategori_related == $category->id_kategori_related) ? 'selected="selected"' : '';?> value="<?= $category->id_kategori_related;?>"><?= $category->nama_kategori;?></option>
                        <?php endforeach; ?>
                      </select>
                    </div>
                  <?php endif; ?>
                  ...
                  <?php else: ?>
                    <div class="form-group">
                      ...
                      <input type="text" name="nama_regulation_ind" class="form-control" required="required" placeholder="Fill in the Regulation name..." value="<?= $datanya->nama_regulation_ind;?>">
                    </div>
                  <?php endif; ?>
                  ...
                  <?php else: ?>
                    <div class="form-group">
                      ...
                      <input type="text" name="nama_regulation_eng" class="form-control" required="required" placeholder="Fill in the Regulation name..." value="<?= $datanya->nama_regulation_eng;?>">
                    </div>
                  <?php endif; ?>
                  <?php if($pdf_ind_error == TRUE OR form_error('pdf_ind') == TRUE): ?>
                    <div class="form-group">
                      ...
                      <input type="file" name="pdf_ind" class="form-control is-warning">
                    </div>
                  <?php else: ?>
                    <div class="form-group">
                      <label>File PDF IND</label>
                      <input type="file" name="pdf_ind" class="form-control">
                    </div>
                  <?php endif; ?>
                ...
                <div class="card-footer">
                  ...
                  <button type="cancel" class="btn btn-danger" onclick="window.location='<?= site_url('backend/geothermalbusiness/relatedregulation');?>';return false;">Batal</button>
                </div>
              </form>
            </div>
        </div>
